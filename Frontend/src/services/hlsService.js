...
// Кэш для плейлистов и сегментов - переносим ЗА пределы объекта
...
export const hlsService = {
    ...
    getMediaPlaylist: async (videoId, quality) => {
        try {
            const response = await fetch(`${HLS_BASE_URL}/videos/${videoId}/${quality}/playlist.m3u8`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            return await response.text();
    ...
    getVideoSegment: async (videoId, quality, segmentName) => {
        try {
            const response = await fetch(`${HLS_BASE_URL}/videos/${videoId}/${quality}/${segmentName}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
        ...
        return class CustomLoader extends Hls.DefaultConfig.loader {
            ...
            getCacheKey(url, type) {
                ...
                if (type === 'segment') {
                    ...
                    return `unknown-${urlParts[urlParts.length - 1]}`;
                } else {
                    // Для плейлистов используем videoId
                    let videoIndex = urlParts.findIndex(part => part === 'streaming');
                    if (videoIndex !== -1) {
                        const videoId = urlParts[videoIndex + 1] || 'master';
                        const playlistType = urlParts[urlParts.length - 2] || 'master';
