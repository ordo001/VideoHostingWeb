...
const WatchPage = () => {
  ...
  // Check if video is available for viewing (must be Approved, or user must be Admin)
  if (videoData?.moderation_status && videoData.moderation_status !== 'Approved' && !user?.isAdmin) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 rounded-full bg-yellow-900 bg-opacity-20 flex items-center justify-center mx-auto mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
          </div>
          <h2 className="text-2xl font-bold text-white mb-4">Видео недоступно</h2>
          <p className="text-gray-400 mb-2">
            {videoData.moderation_status === 'Pending'
              ? 'Это видео проходит модерацию и пока недоступно для просмотра.'
              : 'Это видео было отклонено модераторами и недоступно для просмотра.'}
          </p>
